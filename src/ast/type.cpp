...
bool TypePatternUnresolvedExpression::PatternMatcher::check(Node* expression,Trait* currentTrait){
	if(expression->asTypeReference()){ //| int32
		...
		if(auto callingUnresolvedFunction = call->object->asUnresolvedSymbol()){
			...
			if(currentTrait == tref->trait || tref->trait->isResolved()){
				numberOfParams = tref->trait->numberOfTemplateParameters();
				resolvedObject = true;
			}
		}
		if(resolvedObject){
			...
			if(auto argTuple = call->arg->asTupleExpression()){
				...
				if(numberOfParams != 0 && numberOfParams != argTuple->size()) return false;
				return true;
			}
			else {
				if(numberOfParams > 1) return false;
...
bool TypePatternUnresolvedExpression::PatternMatcher::match(Node* object,Node* pattern){
	...
	if(auto type2 = pattern->asTypeReference()) return type->isSame(type2->type); //| int32
		...
		if(auto callingUnresolvedFunction = call->object->asUnresolvedSymbol()){
			...
			std::vector<Node*> traitExpansions;
			if(tref->trait->templateDeclaration) traitExpansions.resize(tref->trait->numberOfTemplateParameters(),nullptr);
			auto matches = typeSatisfiesTrait(expansionScope,type,tref->trait,resolver,&traitExpansions);
